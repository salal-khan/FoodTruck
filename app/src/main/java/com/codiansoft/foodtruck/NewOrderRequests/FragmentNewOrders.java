                        ...
                        try {
                            ...
                            if (result.get("status").equals("success")) {
                                ...
                                if (Data12.length() > 0) {
                                    ...
                                    adapter = new GenreAdapter(usersorderses, getActivity(), FragmentNewOrders.this);
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (result.get("status").equals("success")) {
                                ...
                                if (Data12.length() > 0) {
                                    long small = 0L;
                                    MyApplication.getInstance().getPrefManger().setLasttime(System.currentTimeMillis());
                                    for (int j = 0; j < Data12.length(); j++) {
                                        JSONObject single = Data12.getJSONObject(j);
                                        if (!single.getString("delivery_time").equals("")) {
                                            if (getDate(single.getString("delivery_time")) > small) {

                                                small = getDate(single.getString("delivery_time"));
                                                MyApplication.getInstance().getPrefManger().setLasttime(getDate(single.getString("delivery_time")));
...
//                        getActivity().finish();
    ...
    public static long getDate(String val) {
        try {
            ...
            final String dateval = "yyyy-MM-dd HH-mm-ss";
